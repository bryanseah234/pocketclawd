...
function sendToDestination(dest: DestinationEntry, body: string, routing: RoutingContext): void {
  ...
  const baseReplyTo = destRouting?.inReplyTo ?? routing.inReplyTo;

  // Fast path: most responses are single-segment (no threaded delimiters).
  const hasDelimiters = body.includes('===REPLY:') && body.includes('===END===');
  if (!hasDelimiters || !body.trim()) {
    writeMessageOut({
      id: generateId(),
      in_reply_to: baseReplyTo,
      kind: 'chat',
  ...
  // Slow path: parse ===REPLY:{id}===...===END=== threaded segments.
  const inboundRefs: InboundRef[] = routing.inReplyTo ? [{ id: routing.inReplyTo }] : [];
  const segments = parseThreadedResponse(body, inboundRefs);
  let prevMsgId: string | null = null;
  for (const seg of segments) {
    const msgId = generateId();
    writeMessageOut({
      id: msgId,
      in_reply_to: prevMsgId ?? baseReplyTo,
      kind: 'chat',
      platform_id: platformId,
      channel_type: channelType,
      thread_id: destRouting?.threadId ?? null,
      content: JSON.stringify({ text: seg.content }),
    });
    prevMsgId = msgId;
  }
}
