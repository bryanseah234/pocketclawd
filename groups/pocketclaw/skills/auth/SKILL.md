...
1. Verify `~/.pocketclaw/secrets/google_credentials.json` exists. If not, instruct the user to:
   - Visit https://console.cloud.google.com → create project "PocketClaw"
   - Enable Gmail API, Calendar API, People API
   - Create OAuth 2.0 client → Desktop app → download `credentials.json`
   - Place it at `~/.pocketclaw/secrets/google_credentials.json`
2. Run the setup script that loads the credentials, opens the browser for consent, and saves the token to `~/.pocketclaw/secrets/google_token.json`.
...
2. Trigger the device-code flow → display the URL + code → save token to `~/.pocketclaw/secrets/ms_token.json`.
