...
// Update WhatsApp Prawn Hub: require @pocketclawd trigger, drop non-matching messages
db.prepare("UPDATE messaging_group_agents SET engage_pattern = '@pocketclawd', ignored_message_policy = 'drop' WHERE id = 'mga-1779356591380-ghyp4q'").run();
console.log('Updated WhatsApp Prawn Hub: trigger=@pocketclawd, policy=drop');
