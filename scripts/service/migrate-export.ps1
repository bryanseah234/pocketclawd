# PocketClaw — Migrate Export
#
# Bundles everything you need to move PocketClaw to a new machine:
#   - .env (credentials)
#   - ~/.pocketclaw/secrets/ (Google + MS + Apple OAuth tokens)
#   - ~/.pocketclaw/vault/ (Obsidian wiki + minutes + research + slides)
...
#   .\scripts\service\migrate-export.ps1                        # writes pocketclaw-export-YYYYMMDD.zip
...
$staging = Join-Path $env:TEMP "pocketclaw-export-$stamp"
$zipPath = Join-Path $OutputDir "pocketclaw-export-$stamp.zip"
...
# --- README ----
$readme = @"
PocketClaw Export — $stamp
Source: $($env:COMPUTERNAME)\$($env:USERNAME)

To restore on a new machine:

1. Clone the pocketclaw repo to the same path style (any X:\... or
   D:\... is fine).
...
The import script will:
  ...
  - Copy secrets/ into ~/.pocketclaw/secrets/
  - Copy vault/ into ~/.pocketclaw/vault/
