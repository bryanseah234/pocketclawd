...
export interface PdpaDependencies {
    ...
    /**
     * Restart-safe store for in-flight consent/deletion flows.
     * Optional — when omitted, the module uses a process-local in-memory
     * fallback (suitable for tests and single-process local runs).
     * In cloud mode, inject RedisPdpaFlowStore so flows survive restarts.
     */
    flowStore?: import('./flow-store.js').PdpaFlowStore;
