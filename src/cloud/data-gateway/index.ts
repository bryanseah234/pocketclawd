...
export class DataGateway implements IDataGateway {
     ...
     */
    async uploadFile(userId: string, bucket: string, key: string, stream: ReadableStream): Promise<string> {
        this.assertUserId(userId);
     ...
     * @param key - Object key — MUST start with `{userId}/`
     */
    async deleteFile(userId: string, bucket: string, key: string): Promise<void> {
        this.assertUserId(userId);
        this.assertKeyBelongsToUser(userId, key);
