...
export class DataGateway implements IDataGateway {
  ...
  async hybridSearch(userId: string, query: string, vector: number[], topK: number): Promise<SearchResult[]> {
        ...
        query: {
          bool: {
            ...
            filter: [
              {
                bool: {
                  should: [{ term: { userId } }, { term: { userId: DataGateway.CORPORATE_SENTINEL } }],
    ...
    const knnHits = (knnResponse.body.hits?.hits ?? []) as unknown as Array<{
      _id: string;
      _score: number;
      _source: Record<string, unknown>;
    }>;

    const bm25Hits = (bm25Response.body.hits?.hits ?? []) as unknown as Array<{
      _id: string;
      _score: number;
      _source: Record<string, unknown>;
    }>;
    ...
    const maxBm25Score = bm25Hits.length > 0 ? Math.max(...bm25Hits.map((h) => h._score)) : 1;

    // Build a map of document id → combined score

    const scoreMap = new Map<
      string,
      { score: number; hit: Record<string, unknown>; source: 'vector' | 'keyword' | 'hybrid' }
    >();
  ...
  async deleteUserDocuments(userId: string, filename?: string): Promise<void> {
    ...
    const hits1 =
      ...
      (sr1 as unknown as { hits?: { hits?: Array<{ _id: string }> } })?.hits?.hits ??
      [];
    if (hits1.length > 0) {
      await this.openSearchClient.bulk({
        body: hits1.flatMap((h: { _id: string }) => [{ delete: { _index: indexName, _id: h._id } }]),
      });
    }
  }
  ...
  > {
    ...
    const result = await this.openSearchClient.search({
      ...
      body: {
        ...
        query: {
          bool: {
            filter: [{ term: { userId } }, { exists: { field: 'sourceUrl' } }],
          },
        },
    ...
    const r = result as {
      body?: { aggregations?: { by_url?: { buckets?: Bucket[] } } };
      aggregations?: { by_url?: { buckets?: Bucket[] } };
    };
    ...
    return buckets
      ...
      .map((b) => {
        ...
        const uploadedAt =
          b.latest?.value_as_string ?? sample.uploadedAt ?? new Date(b.latest?.value ?? Date.now()).toISOString();
  ...
  async deleteIngestedUrl(userId: string, url: string): Promise<number> {
    ...
    const sr2 = await this.openSearchClient.search({
      ...
      size: 1000,
      body: {
        query: { bool: { filter: [{ term: { userId } }, { term: { 'sourceUrl.keyword': url } }] } },
        _source: false,
      },
    });
    const hits2 =
      ...
      (sr2 as unknown as { hits?: { hits?: Array<{ _id: string }> } })?.hits?.hits ??
      [];
    if (hits2.length === 0) return 0;
    await this.openSearchClient.bulk({
      body: hits2.flatMap((h: { _id: string }) => [{ delete: { _index: indexName, _id: h._id } }]),
    });
    return hits2.length;
  }
  ...
  async uploadDraft(
    ...
    contentType: string,
  ): Promise<{ key: string; url: string; bucket: string; expiresInSec: number }> {
    this.assertUserId(userId);
  ...
  private assertUserId(userId: string): void {
    ...
    if (userId === DataGateway.CORPORATE_SENTINEL) {
      throw new Error(
        'DataGateway: CORPORATE sentinel cannot be used as a regular userId (data isolation enforcement)',
      );
    }
  }
