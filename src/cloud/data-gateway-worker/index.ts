...
async function processNextRequest(services: CloudServices): Promise<boolean> {
    ...
    try {
        ...
        switch (action) {
            ...
            case 'index_s3_object':

                await handleIndexS3Object(services, userId, request);
            ...
            case 'delete_all_user_data':
                ...
                await handleDeleteAllUserData(services, userId, request);
...
async function handleIndexS3Object(
    services: CloudServices,
    userId: string,
    request: Record<string, unknown>,
): Promise<void> {
...
async function handleDeleteAllUserData(
    ...
    await services.dataGateway.deleteUserDocuments(userId);
    log.info('All user data deleted via /forget', { userId });
}
